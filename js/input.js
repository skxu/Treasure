...
(function() {
  ...
  var CanvasInput = window.CanvasInput = function(o) {
    ...
    self._mouseDownHandler = function(e) {
        e = e || window.event;
        self.mousedown(e, self);
      };
    self._mouseUpHandler = function(e) {
        e = e || window.event;
        self.mouseup(e, self);
      };

    self._mouseUpBlur = function(e) {
      e = e || window.event;
    ...
    if (self._canvas) {
      self._canvas.addEventListener('mousemove', self._mouseMoveHandler, false);

      self._canvas.addEventListener('mousedown', self._mouseDownHandler, false);

      self._canvas.addEventListener('mouseup', self._mouseUpHandler, false);
    }

    // setup a global mouseup to blur the input outside of the canvas
    window.addEventListener('mouseup', self._mouseUpBlur, true);
    
    // create the hidden input element
    self._hiddenInput = document.createElement('input');
    self._hiddenInput.type = 'text';
    self._hiddenInput.id = self._id;
    ...
    self._hiddenInput.addEventListener('keydown', self._keyDownHandler);

    // setup the keyup listener
    self._hiddenInput.addEventListener('keyup', self._keyUpHandler);
  ...
  CanvasInput.prototype = {
    ...
    remove: function() {
      ...
      window.clearInterval(self._cursorInterval);
      self._canvas.removeEventListener('mousedown', self._mouseDownHandler, false);
      self._canvas.removeEventListener('mousemove', self._mouseMoveHandler, false);
      self._canvas.removeEventListener('mouseup', self._mouseUpHandler, false);
      self._hiddenInput.removeEventListener('keydown', self._keyDownHandler);
      self._hiddenInput.removeEventListener('keyup', self._keyUpHandler);
      window.removeEventListener('mouseup', self._mouseUpBlur, true);
      var hiddenInputElement = document.getElementById(self._id);
      document.getElementById(self._id).parentNode.removeChild(hiddenInputElement);
      self.mousemove = null;
      self._canvas = null;
      self._ctx = null;
      self._x = null;
      ...
      self._fontColor = null;
      self._placeHolderColor = null;
      self._fontWeight = null;
      self._fontStyle = null;
      self._readonly = null;
      self._maxlength = null;
      self._width = null;
      self._height = null;
      self._padding = null;
      self._borderWidth = null;
      self._borderColor = null;
      self._borderRadius = null;
      self._backgroundImage = null;
      self._boxShadow = null;
      self._innerShadow = null;
      self._selectionColor = null;
